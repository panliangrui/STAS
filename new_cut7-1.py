...
def tiling(slide_filepath, magnification, patch_size, scale_factor=32, tissue_thresh=0.35, method='adaptive',
           overview_level=5,
           coord_dir=None, overview_dir=None, mask_dir=None, patch_dir=None, filename=None, model_final=None, model_swim=None,
           thumbnail_dir=None):
    feature = {}
    feature1 = {}
    ...
    for row in range(num_step_y):
        for col in range(num_step_x):
            ...
            if keep_patch(patch_mask, tissue_thresh, color_bg):
                ...
                #     cv2.rectangle(thumbnail, points_thumbnail[0], points_thumbnail[1], color=(255, 255, 255),
                #                   thickness=3)
                if patch_dir is not None:
                    ...
                    compressed_image = F.interpolate(inputs, size=(224, 224), mode='bilinear', align_corners=False)
                    x, _ = model_final(compressed_image)
                    compressed_image = F.interpolate(inputs, size=(224, 224), mode='bilinear', align_corners=False)
                    y= model_swim(compressed_image)
                    feature[str(row) + '_' + str(col)] = x.cpu().detach().numpy().squeeze()
                    feature1[str(row) + '_' + str(col)] = y.cpu().detach().numpy().squeeze()
                    i += 1
    ...
    if thumbnail is not None:
        rgb_mask = Image.fromarray(np.uint8(thumbnail))
        # rgb_mask.save(mask_filepath)
        rgb_mask.save(f"{overview_dir}/{filename}.png")
...
def run(args):
    ...
    model.eval()
    model.features = nn.Sequential(model.features, nn.AdaptiveAvgPool2d(output_size=(1, 1))) #.AdaptiveAvgPool2d(output_size=(1, 1)
    ...
    model_final.load_state_dict(torch.load('./KimiaNetPyTorchWeights.pth'), strict=False)  # KimiaNetPyTorchWeights.pth

    model_swim = ctranspath()
    model_swim = model_swim.to(device)
    model_swim.head = nn.Identity()
    td = torch.load(r'./ctranspath.pth')
    model_swim.load_state_dict(td['model'], strict=False)

    # # 加载权重文件（注意：这里的map_location参数可根据你的实际情况进行调整）
    # state_dict = torch.load('../densenet121-a639ec97.pth', map_location='cpu')
    # # 逐个匹配模型中存在的键并更新键名
    # new_state_dict = {}
    # for k, v in state_dict.items():
    #     if k.startswith('features'):
    #         new_key = 'module.model.0.' + k[15:]
    ...
    np_dir1 = save_dir / 'np_swim'
    np_dir1.mkdir(parents=True, exist_ok=True)
    ...
    # all_slide_name = joblib.load('../data_split/luad_split.pkl')
    all_slide_name = joblib.load('./stas_patients.pkl')

    new_silde_path_lists = []
    for path in slide_filepath_list:
        filename = os.path.basename(path)
        value = "-".join(filename.split("-")[:10])[:-4] #4
        # for item in all_slide_name:
        #     if item + "-01Z" == value:
        #         new_silde_path_lists.append(path)
        for item in all_slide_name:
            if item == value:
                new_silde_path_lists.append(path)

    num_slide = len(new_silde_path_lists)
    print(f"Slide number: {num_slide}.\n")
    print(f"Start tiling ...")

    for slide_idx, slide_filepath in enumerate(new_silde_path_lists):

        print(str(slide_filepath))
        # 获取文件名
        filename = os.path.splitext(os.path.basename(slide_filepath))[0]

        # 提取需要的部分
        # result = "-".join(filename.split("-")[:3])
        # if str(result) in all_slide_name:
        ...
        joblib.dump(one_fea1, str(np_dir1) + '/' + filename + ".pkl")
    ...
    #     # 提取需要的部分
    #     result = "-".join(filename.split("-")[:3])
    #     if result in all_slide_name[0][1]:
    #     # b = all_slide_name[0]
    #     # A = str(slide_filepath).split('\\')[-1].split('.')[0]
    #     # if str(slide_filepath).split('\\')[-1].split('.')[0] in all_slide_name:
    #
    #
    #         if args.specify_filename:
...
def main():
    ...
    parser.add_argument('--slide_dir', type=str, default='./upload_svs')
    parser.add_argument('--save_dir', type=str, default='./stas_all')
