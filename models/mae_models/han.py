...
import dgl
from models.mae_models.gat import GATConv
...
class HANLayer(nn.Module):
    ...
    def __init__(self, num_metapath, in_dim, out_dim, nhead,
        ...
        for i in range(2):
            self.gat_layers.append(GATConv(in_dim, out_dim))#, nhead,feat_drop, attn_drop, negative_slope, residual, activation, norm=norm, concat_out=concat_out))
        self.semantic_attention = SemanticAttention(in_size=512)
    ...
    def forward(self, gs, h):
        ...
        for gat_layer in self.gat_layers:
            x = gat_layer(h, gs)
            semantic_embeddings.append(x)
        # semantic_embeddings.append(self.gat_layers(gs, h))#.flatten(1))  # flatten because of att heads
